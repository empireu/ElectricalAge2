...
open class MultipleFluidTank(val capacity: Int, val requireThermalFluid: Boolean) : IFluidHandler {
    ...
    private fun isFluidAllowed(fluid: Fluid) : Boolean {
        ...
        return ThermalFluidManager.getThermalFluid(fluid) != null
    }
...
class GravityBasedMultipleFluidTank(val parent: MultipleFluidTank) : IFluidHandler by parent {
    ...
    fun updatePhases() {
        ...
        phases.forEachIndexed { index, phase ->
            ...
            phase.forEach { stack ->
                mass += !ThermalFluidManager.requireThermalFluid(stack.fluid).density * stack.amount
                volume += stack.amount
            }
    ...
    override fun drain(resource: FluidStack, action: IFluidHandler.FluidAction): FluidStack {
        ...
        return parent.drain(targetStack, action)
    }
